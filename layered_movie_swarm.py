...
def run_selftest():
    ...
    shared_codebook = len(swarm.normal)                    # organism: one shared codebook across all movies
    dedup = per_movie_codebooks / shared_codebook
    ...
    assert dedup > 1.5 and 0 < per_dim_err < cell
    ...
    print(f"""
{"="*94}
 VERDICT — 144p base + swarm codebook -> more movies, honestly LOSSY:
 * YES, more movies fit (~{movies_layered:.0f} vs 4) — but the win is ENTIRELY the 144p BASE ({px720//px144}x fewer pixels, a resolution
   fact, not the organism; the codebook moves the count by ~0.2 of a movie), and the movies are LOSSY reconstructions
   (codebook error {per_dim_err:.3f}/dim vs cell {cell:.3f}, measured on SYNTHETIC patches, and the dominant 144p->720p upscale
   loss is NOT measured here), NOT the true 720p. Scalable coding + codebook super-resolution — a known-lossy technique.
 * The ORGANISM's real, measured role is the SHARED ADAPTIVE DEDUP CODEBOOK STORE: {dedup:.1f}x less than per-movie codebooks,
   adapts to new content online, regenerates byte-exact. The VQ (patch->key) and the device's upscale/assembly are
   NOT the organism.
 * HONEST: the extra movies are approximations, not originals; the saving needs cross-movie pattern sharing + accepting
   the loss; the organism does the dedup/sharing/adapt, the device does the computing/assembly. The life makes it easier.
{"="*94}""")
